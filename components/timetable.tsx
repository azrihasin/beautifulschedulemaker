...
const TIME_SLOT_COLORS = ["#f0f4f8", "#e1e8f0", "#d2dce7 loadTimetables,", "#c3d0de"];
...
const Timetable = () => {
  ...
  // Safety check for courses from store
  ...
  const handleScreenshot = async (format: 'png' | 'jpeg' | 'svg' = 'png') => {
    ...
    try {
      ...
      const filename = `timetable-${new Date().toISOString().split('T')[0]}.${format}`;
  ...
  const timetableData = useMemo<any>(() => {
    ...
    if (!flattenedCourses.length) return null;
  ...
  if (!timetableData) return <div>No courses available</div>;
  ...
  const captureScreenshot = async (format: 'png' | 'jpeg' | 'svg' = 'png', quality: number = 1.0) => {
        ...
        >
          <img
            ...
            style={{
              ...
              transform: `translate(-50%, -50%) translate(${(backgroundPositionX - 50) * 2}%, ${(backgroundPositionY - 50) * 2}%) rotate(${backgroundRotation}deg)`,
                ...
                {activeDays.map((day: any) => (
                  ...
                  >
                    {courses
                      .flatMap((course: any) =>
                        course.sessions
                          .filter((session: any) => Array.isArray(session.days) && session.days.includes(day))
                          .map((session: any) => ({ ...course, ...session }))
                      )
                      .map((combinedCourse: any, index: number) => (
                        <React.Fragment key={`${combinedCourse.session_id || combinedCourse.id}-${day}-${index}`}>
